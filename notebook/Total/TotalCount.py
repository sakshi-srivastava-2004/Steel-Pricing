...
# The consolidated_output_file_name variable is no longer used for writing to file,
# but can remain if you want to reuse the path string for other purposes later.
# For this request, the file will NOT be created.
...
def calculate_weekly_averages_with_regions(df_input, date_col_name, material_col_name, region_col_name, price_col_name, week_col_name=None):
    ...
    # This section prints the number of weeks for each group BEFORE final capping
    print("\n        --- Weekly Data Counts Before Final Truncation/Capping ---")
    if not result_df.empty:
        for (mat, reg), group in result_df.groupby(['material', 'region']):
            num_weeks = len(group['Week'].unique())
            print(f"        Material: '{mat}', Region: '{reg}': {num_weeks} unique weeks")
    else:
        print("        No weekly data generated after initial processing (before capping).")
    print("        ------------------------------------------------------------")

    # Ensure no more than 53 weeks per material-region combination
    final_result_dfs = []
    for (mat, reg), group in result_df.groupby(['material', 'region']):
        # If the number of unique weeks exceeds 53, truncate to the earliest 53
        if len(group['Week'].unique()) > 53:
            print(f"        Warning: Material '{mat}', Region '{reg}' has more than 53 unique weeks ({len(group['Week'].unique())}). Truncating to 53 weeks (keeping earliest).")
            # Get the top 53 weeks based on the week number (ascending order)
            sorted_group = group.sort_values(by='Week')
            # Select the first 53 unique weeks
            top_weeks_to_keep = sorted_group['Week'].unique()[:53]
            final_result_dfs.append(sorted_group[sorted_group['Week'].isin(top_weeks_to_keep)])
        else:
            # If 53 weeks or fewer, keep as is
...
def main():
    ...
    # --- Final Step: Print all consolidated data to the command window ---
    if final_processed_data_frames:
        ...
        print("\n" + "="*50)
        print("Final Consolidated Processed Data:")
        print("="*50)
        # Set display options to ensure full DataFrame is printed
        ...
        print(f"\nNo data was processed from any file.")
